...
public class CacheInput {
     ...
     */
    public static Optional<CacheInput> current() { return Optional.ofNullable(current.get()); }
    public static CacheInput get() { return current().orElseGet(CacheInput::new); }
    ...
    public synchronized boolean inconsistent() { return inconsistent; }
    ...
    public synchronized Map<BinaryCache, CacheSnapshot> snapshots() { return frozen ? snapshots : new HashMap<>(snapshots); }
    private void modify() {
        if (frozen)
    ...
    public synchronized Map<String, Object> parameters() { return frozen ? parameters : new HashMap<>(parameters); }
    @SuppressWarnings("unchecked") public synchronized <T> T parameter(String key) {
    ...
    @SuppressWarnings("unchecked") public synchronized <T> T parameter(String key, Supplier<T> supplier) {
    ...
    private String hash;
    public synchronized String hash() { return frozen ? hash : hashId(toString()); }
    public CacheStability stability() {
        ...
        /*
         * Changes in stability of dependencies occur concurrently with changes in tracked cache input.
         * We can then end up with input object that references null snapshot of an empty dependency cache
         * while stability state of that dependency has already switched to ready,
         * because the dependency cache was populated in another thread.
         * We cannot allow cache refresh in this scenario, because the refresh would fail with either empty cache exception
         * or "cache input cannot be modified anymore" depending on how the cache reads its dependencies.
         * We therefore have to indicate the cache is still unstable and wait for cache input to be reevaluated.
         */
        if (snapshots.values().stream().anyMatch(Objects::isNull))
            return CacheStability.UNSTABLE;
        /*
    ...
    public static CacheInput of(BinaryCache cache) { return CacheOwner.of(cache).input.get(); }
}
