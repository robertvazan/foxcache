...
public class CacheWorker {
    ...
    public static CacheWorker of(BinaryCache cache) { return CacheOwner.of(cache).worker; }
    public BinaryCache cache() { return owner.cache; }
    ...
    private final ReactiveVariable<Instant> started = OwnerTrace
        ...
        .tag("role", "started")
        .target();
    public Instant started() { return started.get(); }
    ...
    private void refresh(Progress progress) {
        try {
            ...
            try {
                ...
                try {
                    logger.info("Refreshing {}.", toShortString());
                    synchronized (this) {
                        /*
                    ...
                    logger.info("Refreshed {}.", toShortString());
                } finally {
                    lock.unlock();
                }
            } catch (Throwable ex) {
                ...
                if (!ExceptionUtils.getThrowableList(ex).stream().anyMatch(x -> x instanceof CancellationException))
                    logger.error("Failed to refresh persistent cache {}.", toShortString(), ex);
                else
                    logger.info("Cancelled refresh of {}.", toShortString());
                synchronized (this) {
                    /*
    ...
    public synchronized void schedule() {
        ...
        if (permitted) {
            ...
            logger.info("Scheduling {}.", toShortString());
    ...
    @Override public synchronized String toString() { return owner.cache.unwrap().toString(); }
    private static final int NAME_LIMIT = 100;
    private static final int NAME_GRACE = 20;
    private String toShortString() {
        var full = toString();
        if (full.length() <= NAME_LIMIT + NAME_GRACE)
            return full;
        return full.substring(0, NAME_LIMIT) + "...";
    }
}
